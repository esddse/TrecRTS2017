...
def expand(day):
    global expand_day
    preday = str(day-1)
    expand_dir = '../data/expand_tf/' + preday + '/'
    for query in query_list:
        # reset
        query.expanded_tf = copy.deepcopy(query.stem_tf)
        # expand
        expand_file = expand_dir + query.id
        expand_candidates = []
        with open(expand_file, 'r') as f:
            for line in f:
                line = line.strip().split('\t')
                expand_candidates.append((line[0], int(line[1])))
        cnt = 0
        for candidate in expand_candidates:
            term = candidate[0]
            if term not in query.expanded_tf:
                query.expanded_tf[term] = alpha
                cnt +=1 
                if cnt == K: break
        print query.id, "stem: ", query.stem_tf
        print query.id, "expand: ", query.expanded_tf
        print ""
...
def tweet_handle(curday, tweet):
    if expand_day != curday:
        expand(curday)
    if not os.path.exists(dirpath+str(curday)):
        os.mkdir(dirpath+str(curday))
    for query in query_list:
        filepath = dirpath + str(curday) + '/' + query.id
...
def main():
    try:
        ...
        rows = None
        while rows != '':
            cur.execute('SELECT * FROM preprocess WHERE is_process=2 LIMIT 100000')
            rows = cur.fetchall()
            ...
            for row in rows:
                ...
                cur.execute('UPDATE preprocess SET is_process=3 WHERE id=%d' % id)
